...
# ALL PATHS THAT WE NEED TO IMPORT MODEL AND CONFIG
...
try:
    with open('config-message/Model.txt','r') as modelFile:
        MODEL_NAME = str(modelFile.read())
except:
    print("Model checkpoint not found with the given name OR")
    print("Model.txt not found in config-message folder")
    quit()
...
# Restore checkpoint (importing model)
ckpt = tf.compat.v2.train.Checkpoint(model=detection_model)
ckpt.restore(os.path.join(CHECKPOINT_PATH, MODEL_NAME)).expect_partial()

# FUNCTION TO PREDICT LABEL OF THE INPUT
@tf.function
def detect_fn(image):
...
"")
        ...
        # Set percentage from file 
        try:
            with open("config-message/percentage.txt",'r') as f:
                self.percentage = int(eval(f.read()))
        ...
        self.btn_send.clicked.connect(self.Send_confrim_message)            #SEND CONFIRM MESSAGE
    ...
    def Send_confrim_message(self):
        ...
        try:
            with open("config-message/file.txt",'w') as file:
                message = self.TextEdit_confirm_message.toPlainText()
                i=0
                space = 1
                new_sentence = ""
                while(len(message)>i):
                    if message[i]==" ":
                        if space%13==0:
                            new_sentence +="\n"
                        space +=1
                    new_sentence += message[i] 
                    i+=1
                file.write(new_sentence)
        except:
            print("file.txt not found in config-message folder")
...
class Worker1(QThread):
    ...
    # ATTRIBUTES
    default = 0                             #default camera set if problem
    camera = 1                              #initial camera
    ImageUpdate = pyqtSignal(QImage)        #Signal of image that is send to main thread
    LabelUpdate = pyqtSignal(['QString'])   #Signal of label that is send to main thread

    # will run when object_name.start() is call
    def run(self):
        ...
        while self.ThreadActive:
            ...
            # This combine label to image and also send to main thread
            if ret:
